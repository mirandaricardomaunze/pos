      ...
      <html>
        <head>
          ...
          <style>
            ...
            .item-name { flex: 2; overflow: hidden; text-overflow: ellipsis; }
        ...
        <body>
          ...
          <div class="divider"></div>
          <div>
            <div class="item-row"><span><strong>Operador:</strong></span><span>${user?.name || '---'}</span></div>
            <div class="item-row"><span><strong>Cliente:</strong></span><span>${clientName || 'Consumidor Final'}</span></div>
          </div>
          <div class="divider"></div>
          <div>
            ${cart.map(item => `
          ...
          <div class="divider"></div>
          <div>
            <div class="item-row"><span>SubTotal:</span><span>Mtn ${subTotal.toFixed(2)}</span></div>
            <div class="item-row"><span>IVA:</span><span>Mtn ${iva.toFixed(2)}</span></div>
            <div class="item-row total-row"><span>TOTAL:</span><span>Mtn ${total.toFixed(2)}</span></div>
          </div>
          <div class="divider"></div>
          <div>
            <div class="item-row"><span>Valor Pago:</span><span>Mtn ${amountPaid.toFixed(2)}</span></div>
            <div class="item-row"><span>Troco:</span><span>Mtn ${change.toFixed(2)}</span></div>
          </div>
          <div class="divider"></div>
          <div class="footer">
  ...
  return (
    <div className="flex flex-col md:flex-row min-h-screen bg-gray-50 print:bg-white p-6 gap-6">
        ...
        <div className="relative bg-white p-5 rounded-xl shadow-sm border border-gray-200">
          ...
          <Input
            ...
            onChange={(e) => {
              ...
              setCurrentPage(1); // Reset page ao pesquisar
            }}
          />
        </div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-5 max-h-[70vh] overflow-y-auto pr-2">
          {paginatedProducts.map((product) => (
            ...
            >
              ...
              <p className="text-blue-600 font-semibold mt-1">Mtn {product.sellingPrice.toFixed(2)}</p>
      ...
      <div className="w-full md:w-1/3 bg-white p-6 rounded-xl shadow-sm border border-gray-200 h-fit sticky top-6">
        {/* ... O restante do carrinho permanece igual ... */}
        ...
        <div className="space-y-3 border-t border-gray-200 pt-4">
          <div className="flex justify-between text-gray-700"><span>SubTotal:</span><span className="font-medium">Mtn {subTotal.toFixed(2)}</span></div>
          <div className="flex justify-between text-gray-700"><span>IVA:</span><span className="font-medium">Mtn {iva.toFixed(2)}</span></div>
          <div className="flex justify-between text-lg font-bold text-gray-900 border-t border-gray-200 pt-3 mt-2">
            <span>Total:</span><span>Mtn {total.toFixed(2)}</span>
          </div>
        </div>
